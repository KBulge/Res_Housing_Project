...
SQL_FILE = Path(__file__).with_name("validate_gold.sql")


def run_validation() -> None:
    """Run Gold data-quality checks and fail the pipeline when necessary."""
    ...
    try:
        ...
        if results.empty:
            raise RuntimeError(
                "Gold validation query returned no results."
            )
        ...
        print("\nGOLD VALIDATION EXECUTION ERROR")
        print("=" * 60)
